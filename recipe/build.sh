...
#   - Optimized build
#     * With newer optimizations (SSE, AVX2, FMA)
#     * Against MKL (but hopefully, against defaults MKL)
#     * With XLA
#     * Probably we are going to lose default jemmalloc (or we need to patch jemmalloc bazel BUILD)
#
# This recipe is heavily inspired on:
#   jjhelmus work for conda-forge:
#   jjhelmus / kalefranz tensorflow-gpu package in anaconda defaults (which is inspired on the latter)
...
# Disable jemmaloc (needs MADV_HUGEPAGE macro which is not in glib <= 2.12)
# Use LD_PRELOAD at runtime
...
# This is not needed, and relocation might be undesiderable
export CUDNN_INSTALL_PATH="${CUDA_TOOLKIT_PATH}"
# export CUDNN_INSTALL_PATH="${PREFIX}"
# mkdir -p ${PREFIX}/lib64
# cp ${PREFIX}/lib/libcudnn* ${PREFIX}/lib64/

# MKL - make sure it links against conda's mkl
# We could also instruct it to download it... see configure
